...
export function normalizeProviderError(error, provider = "unknown") {
  ...
  const serviceName = provider === "chatgpt-web" ? "ChatGPT 网页" : provider === "local-codex" ? "本机 Codex" : "AI 服务";
  ...
  if (status === 429 && (rawCode.includes("quota") || message.includes("quota") || message.includes("billing"))) {
    return new ProviderError(`${serviceName} 当前额度窗口不可用，请等待重置后重试`, { code: "quota_exceeded", status: 429, provider, cause: error });
  }
  if (status === 429) {
